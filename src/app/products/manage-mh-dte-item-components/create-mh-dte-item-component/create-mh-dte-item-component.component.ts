...
@Component({
  selector: 'mifosx-create-mh-dte-item-component',
  templateUrl: './create-mh-dte-item-component.component.html',
  styleUrls: ['./create-mh-dte-item-component.component.scss'],
  imports: [
...
export class CreateMhDteItemComponentComponent implements OnInit {
  ...
  submit() {
    ...
    this.productsService.createMhDteItemComponent(payload).subscribe({
      next: (res: any) => {
        this.mhDteItemComponentsListRefreshService.setShouldRefresh();
        const newId = res?.id ?? res?.resourceId;
        if (newId != null && newId !== '') {
